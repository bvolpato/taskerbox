		...
		comboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Logger.getRootLogger().setLevel(
						Level.toLevel((String) comboBox.getSelectedItem()));
				log.info("Selected: " + comboBox.getSelectedItem());
			}
		});
		comboBox.setModel(new DefaultComboBoxModel<String>(new String[] { "DEBUG",
				"INFO", "WARN" }));
			...
			synchronized (box.getChannels()) {
				...
				for (TaskerboxChannel<?> channel : box.getChannels()) {
					...
					if (channel.isDaemon()) {
						...
						checkboxThread.setText(channel.getDisplayName() + " (" + (channel.isRunning() ? "running" : "wait") + " #" + channel.getCheckCount() + ")");
					}
