...
public class LinkedInJobDBComparer {
	...
	private static final TermScorer[] TERMS = {
		new TermScorer("h1b", 10), 
		new TermScorer("java", 10), 
		new TermScorer("openedge", 20), 
		new TermScorer("java ee", 10),
		new TermScorer("j2ee", 10),
		new TermScorer("ejb", 8),
		new TermScorer("flex", 10),
		new TermScorer("jsp", 10),
		new TermScorer("jsf", 2),
		new TermScorer("jpa", 8),
		new TermScorer("svn", 8),
		new TermScorer("git", 2),
		new TermScorer("spring", 5),
		new TermScorer("jax-ws", 8),
		new TermScorer("jax-rs", 8),
		new TermScorer("lucene", 8),
		new TermScorer("junit", 10),
		new TermScorer("jenkins", 10),
		new TermScorer("hudson", 10),
		new TermScorer("ant", 10),
		new TermScorer("maven", 10),
		new TermScorer("eclipse", 8),
		new TermScorer("xml", 5),
		new TermScorer("xslt", 5),
		new TermScorer("soap", 5),
		new TermScorer("rest", 5),
		new TermScorer("wsdl", 5),
		new TermScorer("netbeans", 5),
				...
				try {
					...
					if (result.getScore() >= 5000) {
						...
						//		&& LinkedInJobSeeker.considerExperienceDescription(content)) {
							System.out.println(jobFile.getName().split("\\.")[0] + " - Score: " + result.getScore()  + " - " + result.getMatches());
						//}
					}
				} catch (IOException e) {
					e.printStackTrace();
